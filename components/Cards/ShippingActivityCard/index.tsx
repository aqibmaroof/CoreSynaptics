...
const ShippingActivityCard = () => {
  return (
    ...
    <div className="card w-full bg-[#f6f6f6] dark:bg-[#1e4742] rounded-xl shadow-2xl p-6 md:p-8 w-full ml-5 mt-5">
      {/* --- Header --- */}
      <h2 className="text-xl font-semibold text-[#183431] dark:text-white mb-6">
        Shipping activity
      </h2>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className=" text-[#183431] dark:text-white font-medium">
                Shamus Tuttle
              </p>
          ...
          <span className="text-xs  text-[#183431] dark:text-white pt-1">
            Tuesday 11:29 AM
          </span>
        </div>
        ...
        {timelineEvents.map((event, index) => (
          <div key={index} className="flex relative">
            ...
            <div className="flex-grow flex justify-between">
              <div className="max-w-xs pr-4">
                <p
                  className={`font-semibold ${
                    event.isComplete
                      ? " text-[#183431] dark:text-white"
                      : " text-[#183431] dark:text-white"
                  }`}
                >
                  {event.status}
                </p>
                <p className="text-sm  text-[#183431] dark:text-white leading-snug">
                  {event.description}
                </p>
              </div>
              ...
              {/* Time Stamp */}
              {event.time && (
                <span className="text-xs  text-[#183431] dark:text-white flex-shrink-0 text-right">
                  {event.time}
                </span>
              )}
            </div>
