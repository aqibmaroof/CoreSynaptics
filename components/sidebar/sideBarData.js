...
// ════════════════════════════════════════════════════════════════════════════
// BUSINESS_MODULES — canonical module catalogue (matches backend module keys).
// Each entry's `key` is what the backend stores in role.modules[].
// `displayName` must exactly match the corresponding sidebarItems `title`.
// ════════════════════════════════════════════════════════════════════════════
export const BUSINESS_MODULES = [
  ...
  {
    key: "scheduling",
    displayName: "Scheduling & Manpower",
    category: "operations",
    sortOrder: 120,
  },
  {
  ...
  {
    key: "daily_rollup",
    displayName: "Daily Field Report",
    category: "operations",
    sortOrder: 156,
  },
  {
    key: "crew",
    displayName: "Crew & Roster",
    category: "operations",
    sortOrder: 157,
  },
  {
  ...
  {
    key: "commissioning",
    displayName: "Commissioning & Testing",
    category: "operations",
    sortOrder: 170,
  },
  {
    key: "rma_rca",
    displayName: "RMA / RCA & Service",
    category: "operations",
    sortOrder: 180,
  },
  {
    key: "meetings",
    displayName: "Meetings & Coordination",
    category: "operations",
    sortOrder: 182,
  },
  {
  ...
  {
    key: "artifacts",
    displayName: "Artifacts & Drawings",
    category: "management",
    sortOrder: 213,
  },
  {
  ...
  {
    key: "documents",
    displayName: "Document Control & Closeout",
    category: "management",
    sortOrder: 210,
  },
  {
...
// ── MODULE_SECTION_MAP ────────────────────────────────────────────────────────
// Maps every BUSINESS_MODULES key → the sidebar section title(s) it unlocks.
// Titles must match `title` in sidebarItems exactly (case normalised at match time).
export const MODULE_SECTION_MAP = {
  // Sales & CRM
  sales: ["Sales & Commercial"],
  leads: ["Sales & Commercial"],
  contacts: ["Sales & Commercial"],
  deals: ["Sales & Commercial"],
  // Projects & scheduling
  projects: ["Project & Program Management", "Reporting & Dashboards"],
  scheduling: ["Scheduling & Manpower"],
  milestones: ["Scheduling & Manpower"],
  tasks: ["Operations", "Field Execution"],
  // Supply chain
  supply_chain: ["Supply Chain"],
  rma_rca: ["Supply Chain"],
  // Field execution
  field_execution: ["Field Execution"],
  commissioning: ["Field Execution"],
  daily_log: ["Field Execution"],
  daily_rollup: ["Field Execution"],
  crew: ["Field Execution", "HR & Resources"],
  // Quality & Safety (separate sections now)
  qa_qc: ["QA / QC"],
  ...
  documents: ["Document Control & Closeout"],
  // Reports / dashboards
  reports: ["Reporting & Dashboards", "Reports"],
  // Admin — unlocks platform-level sections
  admin: [
    "Reporting & Dashboards",
    "Settings",
    "HR & Resources",
...
// Sidebar top-level titles (lowercase) — used as substring fallback in resolver.
...
/**
 * Converts an array of backend module keys into the Set of sidebar section
 * titles (lowercase) the role may see.
 *
 * Returns null when modules is empty/absent → caller shows all role-gated items.
 */
export function resolveAccessibleSections(modules) {
  if (!Array.isArray(modules) || modules.length === 0) return null;

  const sections = new Set(["reporting & dashboards", "settings"]);
  for (const mod of modules) {
    ...
    const key = mod
      ...
      .trim()
      .replace(/[\s-]+/g, "_");
    const mapped = MODULE_SECTION_MAP[key];
    if (mapped) {
      mapped.forEach((s) => sections.add(s.toLowerCase()));
      continue;
    }
    for (const title of SIDEBAR_SECTION_TITLES) {
      if (title.includes(key) || key.includes(title.replace(/\s+/g, "_"))) {
        sections.add(title);
      }
    }
  }
...
/**
 * Returns sidebar items visible for `role`, optionally filtered by the role's
 * accessible module keys (array of BUSINESS_MODULES.key values from the backend).
 *
 * Filter layers:
 *  1. Role gate — item.roles must include the role string.
 *  2. QAQC gate — isQAQCRole(role) limits to QAQC_ALLOWED_SECTIONS.
 *  3. Module gate — resolveAccessibleSections converts keys → titles; hides rest.
 *
 * When accessibleModules is empty/absent the module gate is skipped (backwards compatible).
 */
export function getMenuByRole(role, accessibleModules) {
  const allowedSections = resolveAccessibleSections(accessibleModules);
  const qaqcUser = isQAQCRole(role);
  return sidebarItems
    .filter((item) => {
