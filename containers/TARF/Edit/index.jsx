...
export default function TARFEdit() {
    ...
    getTARFById(editId)
      .then((res) => {
        ...
        setForm({
          ...
          roleOnSite: d.roleOnSite ?? "",
          expectedStart: d.expectedStart ? d.expectedStart.slice(0, 10) : "",
          expectedEnd: d.expectedEnd ? d.expectedEnd.slice(0, 10) : "",
          // safetyOrientationComplete: d.safetyOrientationComplete ?? false,
        });
      })
      .catch(() =>
        setFetchError("Failed to load TARF record. It may have been deleted."),
      )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "TARF request updated successfully",
      });
      setTimeout(() => router.push("/TARF/List"), 1500);
    } catch (err) {
      setMessage({
        type: "error",
        text: err?.message || "Failed to update TARF request",
      });
    } finally {
      setLoading(false);
    }
          ...
          <div className="mb-6 bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-4 flex gap-3">
            ...
            <div className="text-sm text-yellow-300/80">
              <strong className="text-yellow-300">
                This TARF has been approved.
              </strong>{" "}
              Only safety orientation status and access dates can typically be
              amended post-approval. Contact the approver for major changes.
            </div>
          </div>
        )}
          ...
          <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 p-6 space-y-6">
            ...
            {/* <div
              className={`flex items-start gap-3 p-4 rounded-lg border transition-colors ${
                form.safetyOrientationComplete
