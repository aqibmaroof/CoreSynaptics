...
export default function ContactsList() {
  ...
  const handleSave = async () => {
    if (!form.firstName || !form.lastName) { setMessage({ type: "error", text: "First and last name are required" }); return; }
    setSaving(true);
    try {
      if (editingContact) {
  ...
  const handleCreateTask = async () => {
    ...
    try {
      await CreateTask({ ...taskForm, contactId: taskContactId });
      setShowTaskModal(false);
      setTaskForm({ name: "", description: "", priority: "Medium", dueDate: "" });
      setMessage({ type: "success", text: "Task created for contact" });
    } catch {
              ...
              <tbody>
                {filteredContacts.map((contact) => (
                  <tr key={contact.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    <td className="px-6 py-4 text-white font-medium text-sm">{contact.firstName} {contact.lastName}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setTaskContactId(contact.id); setShowTaskModal(true); }} className="text-green-400 hover:text-green-300 text-xs">+ Task</button>
                ...
                {filteredContacts.length === 0 && (
                  <tr><td colSpan={6} className="text-center text-gray-500 py-12">No contacts found</td></tr>
                )}
              </tbody>
            </table>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })}
                    className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-cyan-500" placeholder="e.g. VP of Engineering" />
                </div>
              </div>
        ...
        {showTaskModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-md w-full">
              <h3 className="text-white font-bold text-lg mb-4">Create Task for Contact</h3>
              <div className="space-y-4">
                <div>
