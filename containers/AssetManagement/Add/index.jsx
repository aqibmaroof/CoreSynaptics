...
export default function AssetAdd() {
  ...
  const validate = () => {
    const e = {};
    if (!form.asset_tag.trim()) e.asset_tag = "Asset tag is required and must be unique";
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      setMsg({ type: "success", text: "Asset registered — initial status: IN_STOCK" });
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="mx-auto">
        ...
        <h1 className="text-4xl font-bold text-white mb-2">Register Asset</h1>
        <p className="text-gray-400 mb-8">
          All new assets start as <span className="text-green-400 font-medium">IN_STOCK</span>.
          Use the list page to assign, send for repair, or retire.
        </p>
          ...
          <section className="bg-gray-900/50 rounded-xl border border-gray-800/50 p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input type="text" value={form.asset_tag} onChange={set("asset_tag")}
                  placeholder="e.g. IT-LAP-0042"
                  className={`${INPUT} font-mono ${errors.asset_tag ? "border-red-500" : ""}`} />
                {errors.asset_tag && <p className="text-red-400 text-xs mt-1">{errors.asset_tag}</p>}
              </div>
              <div>
                <FL>Serial Number</FL>
