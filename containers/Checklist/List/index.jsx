...
function getStatusColor(status) {
  switch (status) {
    case "NOT_STARTED": return "bg-gray-900/30 text-gray-300 border-gray-700/30";
    case "IN_PROGRESS": return "bg-blue-900/30 text-blue-300 border-blue-700/30";
    case "COMPLETED":   return "bg-green-900/30 text-green-300 border-green-700/30";
    case "VERIFIED":    return "bg-purple-900/30 text-purple-300 border-purple-700/30";
    default:            return "bg-gray-900/30 text-gray-300 border-gray-700/30";
  }
}

function toArray(data) {
  return Array.isArray(data) ? data : (data?.data ?? data?.projects ?? data?.sites ?? data?.zones ?? data?.equipment ?? []);
}
...
export default function ChecklistList() {
  ...
  useEffect(() => {
    setSites([]); setSelSite("");
    setSubProjects([]); setSelSubProject("");
    setZones([]); setSelZone("");
    setAssets([]); setSelAsset("");
  ...
  const fetchChecklists = async () => {
    if (!activeFilter) { setError("Select at least a project to load checklists."); return; }
    try {
      setLoading(true);
      setError("");
      const data = await getChecklists({ [activeFilter.key]: activeFilter.value });
      ...
      <div className="mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {activeFilter && (
              <span className="text-xs text-blue-400 bg-blue-900/20 border border-blue-700/30 px-2.5 py-1 rounded-full">
                Filtering by: <span className="font-semibold">{activeFilter.key}</span>
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <svg className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-gray-400" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9" /></svg>
              </div>
            </div>
            ...
            {/* Sub-Project */}
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Sub-Project</label>
              <div className="relative">
                <select
                  value={selSubProject}
          ...
          <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div className="flex items-end">
                <button
                  onClick={() => { setSearchTerm(""); setFilterPhase(""); setFilterType(""); setFilterStatus(""); }}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm">Select a project (and optionally drill down) then click Load Checklists</p>
          </div>
            ...
            {filteredChecklists.map((checklist) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1 min-w-0 mr-3">
                    <p className="text-xs text-gray-500 mb-1">
                      {checklist.checklistType || "—"} · Phase {checklist.phase || "—"}
                    </p>
                    <h3 className="text-base font-semibold text-white group-hover:text-blue-300 transition-colors truncate">
                      {checklist.title}
                    </h3>
                    {checklist.description && (
                      <p className="text-xs text-gray-400 mt-1 line-clamp-2">{checklist.description}</p>
                    )}
                  </div>
                  <span className={`shrink-0 px-2.5 py-1 text-xs font-medium rounded-full border ${getStatusColor(checklist.status)}`}>
                ...
                <div className="flex gap-2 pt-4 border-t border-gray-700">
                  <button
                    onClick={(e) => { e.stopPropagation(); router.push(`/Checklist/Edit/${checklist.id}`); }}
                  ...
                  {!checklist.lockedAt && (
                    <button
                      onClick={(e) => { e.stopPropagation(); setDeleteConfirm(checklist.id); }}
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-xl border border-gray-700 max-w-sm mx-4 shadow-2xl p-6">
              <h3 className="text-lg font-semibold text-white mb-2">Delete Checklist?</h3>
