...
const ComingSoon = () => {
  ...
  const { type } = params; // expects "success" or "error"
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="bg-[url('/images/mainBackground.png')] bg-cover bg-center bg-no-repeat flex flex-col justify-center w-full p-8 md:p-16 lg:p-12 xl:p-20">
        ...
        {/* Status Card */}
        <div className="flex flex-col items-start justify-center flex-1 py-4">
          <div
            className={`
              relative max-w-md w-full rounded-3xl p-10 backdrop-blur-sm border
              ${
                isSuccess
                  ? "bg-white/10 border-[#0075F8]/40"
          ...
          >
            ...
            <div
              className={`
                w-20 h-20 rounded-full flex items-center justify-center mb-8 shadow-lg
                ${isSuccess ? "bg-[#0075F8]" : "bg-red-500"}
              `}
            >
              {isSuccess ? (
                // Checkmark
            ...
            {/* Heading */}
            <h2 className="text-white text-[22px] md:text-[30px] leading-snug mb-2">
              {isSuccess ? "You're all set!" : "Something went wrong"}
              <p className="-mt-1">
                <span
                  className={`text-[40px] md:text-[64px] font-bold scotchItalic leading-none ${
                    isSuccess ? "text-[#0075F8]" : "text-red-400"
                  }`}
                >
                  {isSuccess ? "Subscribed!" : "Try again"}
                </span>
              </p>
            </h2>

            {/* Description */}
            <p className="text-white/80 text-[14px] md:text-[17px] mt-3 mb-8 leading-relaxed">
              {isSuccess
                ? "Thanks for subscribing! We'll send you updates and exclusive promotions as we get closer to launch."
                : "We couldn't process your submission. This might be a temporary issue. Please check your connection and try again."}
            </p>

            {/* Divider */}
            <div
              className={`h-px w-full mb-8 ${
                isSuccess ? "bg-[#0075F8]/30" : "bg-red-400/30"
              }`}
            />
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {isSuccess ? (
                <a
                  href="/"
                  className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#0075F8] to-[#00387A]  text-white font-semibold px-7 py-4 rounded-2xl transition-colors duration-200 text-[15px]"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-4 h-4"
                ...
                <>
                  <a
                    href="/"
                    className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#0075F8] text-white font-semibold w-full py-4 rounded-2xl transition-colors duration-200 text-[15px]"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                  ...
                  <a
                    href="mailto:support@wildtag.com"
                    className="inline-flex items-center w-full justify-center gap-2  border border-white/30 hover:border-white/60 text-white font-semibold py-4 rounded-2xl transition-colors duration-200 text-[15px]"
                  >
                    Contact Support
                  </a>
                </>
