...
import { getOrgSafetyPlan, updateOrgSafetyPlan } from "@/services/OrgSafetyPlans";

export default function OrgSafetyPlanEdit() {
  ...
  useEffect(() => {
    ...
    getOrgSafetyPlan(id)
      .then((res) => {
        ...
        setForm({
          name: d.name ?? "",
          safetyManagerRole: d.safetyManagerRole ?? "",
          requiredPpe: d.requiredPpe ?? "",
          emergencyPlan: d.emergencyPlan ?? "",
          toolboxTalkFrequency: d.toolboxTalkFrequency ?? "",
          nfpa70eRequired: d.nfpa70eRequired ?? false,
          osha30Required: d.osha30Required ?? false,
          osha10Required: d.osha10Required ?? true,
  ...
  const Check = ({ id: cid, label, checked, onChange }) => (
    <div className="flex items-center gap-3">
      <input type="checkbox" id={cid} checked={checked} onChange={onChange} className="w-4 h-4 accent-blue-500" />
      <label htmlFor={cid} className="text-sm text-gray-300">{label}</label>
    </div>
  );
  ...
  return (
    <div className="min-h-screen text-white p-6">
      <div className="mx-auto">
        ...
        <h1 className="text-2xl font-bold mb-6">Edit Safety Plan Template</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-blue-500"
                value={form.safetyManagerRole} onChange={(e) => set("safetyManagerRole", e.target.value)} />
            </div>
            <div>
              <label className="block text-sm text-gray-300 mb-1">Toolbox Talk Frequency</label>
              <input className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-blue-500"
                value={form.toolboxTalkFrequency} onChange={(e) => set("toolboxTalkFrequency", e.target.value)} />
            </div>
          </div>
          <div>
            <label className="block text-sm text-gray-300 mb-1">Required PPE</label>
            <input className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-blue-500"
              value={form.requiredPpe} onChange={(e) => set("requiredPpe", e.target.value)} />
          </div>
          <div>
