...
import { getSuppliers, deleteSupplier } from "@/services/Inventory";

const Toast = ({ msg }) =>
  msg ? (
    <div className={`fixed top-6 right-6 z-50 px-4 py-3 rounded-lg border shadow-xl text-sm ${
      msg.type === "success"
      ...
      <div className="mx-auto">
        ...
        <div className="mb-8 flex justify-between items-start flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400">Manage vendors and suppliers you purchase inventory from</p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 p-16 text-center">
            ...
            <p className="text-gray-600 text-sm mb-6">Add your first supplier to start tracking your vendors</p>
            <button
              onClick={() => router.push("/Inventory/Suppliers/Add")}
            ...
            {filtered.map((sup) => (
              ...
              >
                ...
                <div className="p-5 border-b border-gray-800/60">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-white font-semibold text-base">{sup.name}</h3>
                        {sup.code && (
                ...
                <div className="p-5 space-y-3">
                  {sup.contactName && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-300 text-xs">{sup.contactName}</span>
                    </div>
                  )}
                  {sup.email && (
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-gray-500 w-20 flex-shrink-0">Email</span>
                      <span className="text-gray-400 text-xs truncate">{sup.email}</span>
                    </div>
                  )}
                  {sup.phone && (
                    <div className="flex items-center gap-2 text-sm">
                  ...
                  {sup.notes && (
                    <div className="flex items-start gap-2 text-sm">
                      ...
                      <span className="text-gray-500 text-xs italic leading-relaxed line-clamp-2">{sup.notes}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="px-5 py-4 border-t border-gray-800/60 flex items-center gap-2">
                  <button
                    onClick={() => router.push(`/Inventory/Suppliers/Edit/${sup.id}`)}
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-sm shadow-2xl p-6">
              <h3 className="text-white font-bold text-lg mb-2">Delete Supplier?</h3>
              <p className="text-gray-400 text-sm mb-2">
                Are you sure you want to delete{" "}
                <span className="text-white font-medium">{deleteConfirm.name}</span>?
              </p>
              <p className="text-red-400 text-xs mb-6">
                This is a soft delete — the record will be deactivated and hidden from active lists.
              </p>
