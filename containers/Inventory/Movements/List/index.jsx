...
const Toast = ({ msg }) =>
  msg ? (
    <div className={`z-50 px-4 py-3 rounded-lg border shadow-xl text-sm flex items-center gap-2 ${
      msg.type === "success"
...
export default function MovementsList() {
  ...
  async function applyDateFilter() {
    setLoading(true);
    try {
      const res = await getStockMovements({ fromDate: dateFrom, toDate: dateTo });
  ...
  const filtered = movements.filter((m) => {
    ...
    const mw = !warehouseFilter || m.warehouseId === warehouseFilter || m.toWarehouseId === warehouseFilter;
    return ms && mt && mw;
  });

  const summary = MOVEMENT_TYPES.reduce((acc, t) => {
    acc[t] = movements.filter((m) => m.type === t).length;
    return acc;
  }, {});
  const totalQtyIn  = movements.filter((m) => ["IN", "RETURN"].includes(m.type)).reduce((s, m) => s + (m.quantity || 0), 0);
  const totalQtyOut = movements.filter((m) => m.type === "OUT").reduce((s, m) => s + (m.quantity || 0), 0);
      ...
      <div className="mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
          {[
            ...
            { label: "Net Qty In", value: `${totalQtyIn - totalQtyOut >= 0 ? "+" : ""}${totalQtyIn - totalQtyOut}`,
              color: totalQtyIn >= totalQtyOut ? "text-green-400" : "text-red-400" },
          ].map((s) => (
        ...
        <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 p-4 mb-5 flex flex-wrap gap-3 items-end">
          <div className="flex flex-col gap-1">
            ...
            <input type="text" placeholder="SKU, reference, notes..."
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={warehouseFilter} onChange={(e) => setWH(e.target.value)}
          ...
          <button onClick={() => { setSearch(""); setTypeFilter(""); setWH(""); setDateFrom(MONTH_START); setDateTo(TODAY); }}
        ...
        {loading ? (
          ...
          <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-800">
                    {["Date", "Type", "SKU", "Warehouse", "To Warehouse", "Qty", "Cost/Unit", "Reference", "Notes"].map((h) => (
                  ...
                  {filtered.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-5 py-4 text-gray-300 text-xs font-mono">{m.skuId || "—"}</td>
                        <td className="px-5 py-4 text-gray-400 text-sm">{whName(m.warehouseId)}</td>
                        <td className="px-5 py-4 text-gray-400 text-sm">
                          {m.toWarehouseId ? whName(m.toWarehouseId) : "—"}
                        </td>
                        <td className="px-5 py-4">
                          <span className={`text-base font-bold ${
                            ["IN", "RETURN"].includes(m.type) ? "text-green-400" :
                            m.type === "OUT"                  ? "text-red-400" :
                            m.type === "ADJUSTMENT"           ? "text-purple-400" : "text-blue-400"
                          }`}>
                            {["IN", "RETURN"].includes(m.type) ? "+" : m.type === "OUT" ? "−" : ""}
                            {m.type === "ADJUSTMENT" && m.adjustmentDelta != null
                              ? (m.adjustmentDelta >= 0 ? `+${m.adjustmentDelta}` : m.adjustmentDelta)
                              : m.quantity}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          {m.referenceId ? (
                            <div>
                              <p className="text-gray-300 text-xs font-mono">{m.referenceId}</p>
                              {m.referenceType && (
                                <p className="text-gray-600 text-xs mt-0.5">{m.referenceType.replace(/_/g, " ")}</p>
                              )}
                            </div>
            ...
            <div className="px-5 py-3 border-t border-gray-800 flex items-center justify-between">
              ...
              <p className="text-gray-500 text-xs">
                Net qty change:{" "}
                <span className={`font-semibold ${totalQtyIn - totalQtyOut >= 0 ? "text-green-400" : "text-red-400"}`}>
