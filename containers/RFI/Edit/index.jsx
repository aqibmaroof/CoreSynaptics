...
export default function RFIEdit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (fd.siteId)
          loads.push(
            getProjects(25, 1, fd.siteId, fd.projectId)
              .then(toArray)
              .catch(() => [])
              .then(setSubProjects),
          );
  ...
  const handleAnswer = async () => {
    if (!officialAnswer.trim()) {
      setError("Official answer is required");
      return;
    }
    setError("");
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "RFI answered — status is now ANSWERED",
      });
    } catch (err) {
    ...
    <div className="min-h-screen p-6">
      <div className="mx-auto">
        ...
        <div className="mb-8 flex items-start justify-between">
          ...
          {currentStatus && (
            <span
              className={`px-3 py-1 text-sm font-medium rounded-full border ${STATUS_COLORS[currentStatus] ?? "bg-gray-700 text-gray-300 border-gray-600"}`}
            >
              {currentStatus}
            </span>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <AppSelect
                    ...
                    disabled={
                      isClosed || !formData.projectId || sites.length === 0
                    }
                  />
                </div>
                <div>
                  ...
                  <AppSelect
                    ...
                    disabled={
                      isClosed || !formData.siteId || subProjects.length === 0
                    }
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <AppSelect
                    ...
                    disabled={
                      isClosed || !formData.subProjectId || zones.length === 0
                    }
                  />
                </div>
                <div>
                  ...
                  <AppSelect
                    ...
                    disabled={
                      isClosed || !formData.zoneId || assets.length === 0
                    }
                  />
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {isClosed && (
                  <p className="text-sm text-gray-500">
                    This RFI is closed and cannot be modified.
                  </p>
                )}
              </div>
            </div>
