...
import { getOrgMobCatalogItem, updateOrgMobCatalogItem } from "@/services/OrgMobCatalog";

export default function OrgMobCatalogEdit() {
  const router = useRouter();
  const { id } = useParams();
  const [form, setForm] = useState({ name: "", category: "", icon: "", defaultQty: "1", defaultUnitCost: "0", formula: "", isActive: true });
    ...
    getOrgMobCatalogItem(id)
      .then((res) => {
        ...
        setForm({
          ...
          category: d.category ?? "",
          icon: d.icon ?? "",
          defaultQty: String(d.defaultQty ?? 1),
          defaultUnitCost: String(d.defaultUnitCost ?? 0),
          formula: d.formula ?? "",
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await updateOrgMobCatalogItem(id, {
        name: form.name.trim(),
  ...
  return (
    <div className="min-h-screen text-white p-6">
      <div className="mx-auto">
        ...
        <h1 className="text-2xl font-bold mb-6">Edit Catalog Item</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Formula</label>
