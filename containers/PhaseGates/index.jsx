...
export default function PhaseGatesPanel() {
  ...
  const handleInitialize = async () => {
    try {
      ...
      setMessage({ type: "success", text: "Phase gates initialized for this project" });
      await loadAll();
    } catch (err) {
  ...
  const handleToggleCondition = async (condition) => {
    ...
    try {
      ...
      setConditions((prev) =>
        prev.map((c) =>
          c.id === condition.id
            ? { ...c, condition_met: newMet, conditionMet: newMet, met_at: newMet ? new Date().toISOString() : null }
            : c
        )
      );

      setMessage({ type: "success", text: `${condition.condition_name || condition.conditionName} ${newMet ? "completed" : "unchecked"}` });
    } catch (err) {
      setMessage({ type: "error", text: err?.message || "Failed to update condition" });
    }
  };
  ...
  const handleAdvancePhase = async () => {
    ...
    try {
      ...
      setMessage({ type: "error", text: err?.message || "Failed to advance phase. Check all blocking conditions." });
    } finally {
      setAdvancing(false);
    }
  };
  ...
  const currentPhase = phaseStatus?.current_phase || phaseStatus?.currentPhase || "L1";
  const currentTransitionKey = getTransitionForPhase(currentPhase);
  const currentTransition = currentTransitionKey ? TRANSITIONS[currentTransitionKey] : null;
    ...
    return (
      <div className="min-h-screen p-6">
        <div className="mx-auto text-center py-20">
          <h2 className="text-2xl font-bold text-white mb-4">No Phase Gates Found</h2>
          <p className="text-gray-400 mb-8">Initialize the phase gate system for this project to begin tracking conditions.</p>
    ...
    return (
      <div className="min-h-screen p-6">
        <div className="max-w-4xl mx-auto">
          <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-6 text-red-400">{error}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="mx-auto">
        {/* Toast */}
        {message && (
          <div className={`fixed top-6 right-6 z-50 px-5 py-3 rounded-lg border shadow-xl text-sm font-medium ${message.type === "success" ? "bg-green-900/90 border-green-500/30 text-green-300" : "bg-red-900/90 border-red-500/30 text-red-300"
            }`}>
            {message.text}
          </div>
        )}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-1">Phase Gate Control</h1>
              <p className="text-gray-400 text-sm">Manage phase progression and gating conditions</p>
            </div>
            <button onClick={handleLoadLog}
        ...
        <div className="mb-8 bg-gray-900/60 rounded-xl border border-gray-800/50 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider">Project Phase Timeline</h2>
            {readiness && (
              <span className="text-xs text-gray-500">Overall: {readiness.overallProgress}% complete</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-0">
            {PHASES.map((phase, idx) => {
              ...
              const isFuture = PHASES.indexOf(phase) > PHASES.indexOf(currentPhase);

              return (
                <div key={phase} className="flex items-center flex-1">
                  {/* Node */}
                  <div className="flex flex-col items-center flex-1">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${isCurrent
                      ...
                      {isPast ? (
                        ...
                        phase === "COMPLETE" ? phase[0] + phase.slice(-1) : phase.replace("_", "-") // Show first letter of phase
                      )}
                    </div>
                    <span className={`text-[10px] mt-2 text-center leading-tight ${isCurrent ? "text-white font-bold" : isPast ? "text-green-400" : "text-gray-600"
                      }`}>
                  ...
                  {/* Connector line */}
                  {idx < PHASES.length - 1 && (
                    <div className={`h-0.5 flex-1 -mx-1 ${isPast ? "bg-green-500/50" : isCurrent ? "bg-gradient-to-r from-cyan-500/50 to-gray-700" : "bg-gray-800"}`} />
                  )}
                </div>
              );
            })}
        ...
        {currentPhase !== "COMPLETE" && readiness && currentTransitionKey && (
          <div className={`mb-8 bg-gray-900/60 rounded-xl border ${phaseBorderColors[currentPhase]} p-6`}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-bold text-white">
                  {currentTransition?.label}: Gate Status
                </h2>
                <p className="text-gray-400 text-sm mt-1">{PHASE_LABELS[currentPhase]}</p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="relative w-16 h-16">
                  <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                      fill="none" stroke={readiness.canAdvance ? "#10b981" : "#06b6d4"} strokeWidth="3"
                      strokeDasharray={`${readiness.transitions[currentTransitionKey]?.progress || 0}, 100`} />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-xs font-bold text-white">{readiness.transitions[currentTransitionKey]?.progress || 0}%</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`px-6 py-3 rounded-lg text-sm font-bold transition-all ${readiness.canAdvance
                ...
                >
                  {readiness.canAdvance ? `Advance to ${readiness.nextPhaseLabel?.split("—")[0]?.trim()}` : "Conditions Pending"}
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {(readiness.transitions[currentTransitionKey]?.details || []).map((detail) => (
                <div
                  key={detail.key}
                  onClick={() => {
                    const cond = conditions.find((c) => (c.condition_key || c.conditionKey) === detail.key && c.transition === currentTransitionKey);
                    if (cond) handleToggleCondition(cond);
                  }}
                  className={`flex items-center gap-4 p-4 rounded-lg border cursor-pointer transition-all ${detail.met
                      ? "bg-green-900/10 border-green-500/20 hover:border-green-500/40"
                      : "bg-gray-800/30 border-gray-700/50 hover:border-gray-600"
                    }`}
                >
                  {/* Checkbox */}
                  <div className={`w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 border-2 transition-all ${detail.met ? "bg-green-500 border-green-500" : "border-gray-600 bg-transparent"
                    }`}>
                    {detail.met && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${detail.met ? "text-green-300 line-through opacity-70" : "text-white"}`}>
                      {detail.name}
                    </p>
                    {detail.metAt && (
                      <p className="text-[10px] text-gray-500 mt-0.5">
                        Completed {new Date(detail.metAt).toLocaleDateString()} {detail.metBy ? `by ${getUserName(detail.metBy)}` : ""}
                      </p>
                    )}
                  </div>
        ...
        {currentPhase === "COMPLETE" && (
          <div className="mb-8 bg-green-900/10 rounded-xl border border-green-500/30 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-green-400 mb-2">Project Complete</h2>
            <p className="text-gray-400">All phase gates have been cleared. Project lifecycle is complete.</p>
          </div>
        )}

        {/* ─── ALL TRANSITIONS OVERVIEW ───────────────────────────────── */}
        <div className="mb-8">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">All Phase Transitions</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(TRANSITIONS).map(([key, trans]) => {
              ...
              const isPast = PHASES.indexOf(trans.to) <= PHASES.indexOf(currentPhase);
              const isFuture = PHASES.indexOf(trans.from) > PHASES.indexOf(currentPhase);

              return (
                <div key={key} className={`rounded-xl border p-4 transition-all ${isActive
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <h3 className={`text-sm font-bold ${isActive ? "text-cyan-400" : isPast ? "text-green-400" : "text-gray-500"}`}>
                      {trans.label}
                    </h3>
                    {isActive && <span className="text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">ACTIVE</span>}
                    {isPast && !isActive && <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 border border-green-500/30">PASSED</span>}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-gray-500">
                      {evaluation?.metConditions || 0}/{evaluation?.totalConditions || 0} conditions
                    </span>
                    {evaluation?.blockingUnmet > 0 && (
                      <span className="text-[10px] text-red-400">{evaluation.blockingUnmet} blocking</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-md w-full shadow-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-white font-bold text-lg">Advance Phase</h3>
                  <p className="text-gray-400 text-sm">{currentTransition?.label}</p>
                </div>
              </div>

              <p className="text-gray-300 text-sm mb-4">
                All blocking conditions have been met. Advancing will move the project from
                <span className="text-cyan-400 font-bold"> {currentPhase}</span> to
                <span className="text-green-400 font-bold"> {readiness?.nextPhase}</span>.
                This action is recorded in the audit log and cannot be undone.
              </p>

              <div className="mb-6">
                <label className="block text-xs font-bold text-gray-400 mb-2 uppercase">Notes (optional)</label>
                <textarea
                  value={advanceNotes}
              ...
              <div className="flex gap-3 justify-end">
                <button
                  onClick={() => { setShowAdvanceConfirm(false); setAdvanceNotes(""); }}
        ...
        {showLogModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-white font-bold text-lg">Phase Advancement Log</h3>
              ...
              {advancementLog.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No advancements recorded yet.</p>
              ) : (
                <div className="space-y-4">
                  {advancementLog.map((entry, idx) => (
                    <div key={entry.id || idx} className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/50">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[11px] text-gray-500">
                          {new Date(entry.advanced_at || entry.advancedAt).toLocaleString()}
                        </span>
                      </div>
                      <p className="text-xs text-gray-400">
                        Advanced by: <span className="text-white">{getUserName(entry.advanced_by || entry.advancedBy)}</span>
                      </p>
                      {(entry.notes) && (
