        ...
        <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-end">
              <button
                onClick={() => { setSearchTerm(""); setFilterType(""); setFilterActive(""); }}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm mt-2">
              {searchTerm || filterType || filterActive ? "Try adjusting your filters" : "Create your first company"}
            </p>
          </div>
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-700/50 border-b border-gray-700">
                  <tr>
                    {["Company Name", "Type", "Region", "Plan", "Subscription", "Created", "Actions"].map((h) => (
                      <th key={h} className="px-6 py-4 text-left text-sm font-semibold text-gray-200 whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {filtered.map((company) => (
                    <tr key={company.id} className="hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 text-xs font-medium rounded-full border ${TYPE_COLORS[company.type] || TYPE_COLORS.OTHER}`}>
                          {company.type ? company.type[0] + company.type.slice(1).toLowerCase() : "—"}
                        </span>
                      </td>

                      <td className="px-6 py-4 text-gray-300">{company.region || "—"}</td>

                      <td className="px-6 py-4">
                        {company.subscriptionPlan ? (
                          <span className={`px-3 py-1 text-xs font-medium rounded-full border ${
                            company.subscriptionPlan?.toLowerCase() === "enterprise"
                              ? "bg-purple-900/30 text-purple-300 border-purple-700/30"
                              : company.subscriptionPlan?.toLowerCase() === "premium"
            ...
            <div className="bg-gray-800 rounded-xl border border-gray-700 max-w-sm mx-4 shadow-2xl">
              <div className="p-6">
                ...
                <p className="text-gray-400 mb-6">
                  This will soft-delete the company. It can be restored by an administrator if needed.
                </p>
