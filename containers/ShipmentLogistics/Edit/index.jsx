...
export default function ShipmentEdit() {
  ...
  useEffect(() => {
    getCarriers()
      .then((r) => setCarriers(Array.isArray(r) ? r : r?.data || []))
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    getShipmentById(editId)
      ...
      .catch(() =>
        setFetchError("Failed to load shipment. It may have been deleted."),
      )
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      setMsg({
        type: "error",
        text: err?.message || "Failed to update shipment",
      });
    } finally {
      setLoading(false);
    }
          ...
          <div className="mb-6 bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-4 flex gap-3">
            ...
            <p className="text-sm text-yellow-300/80">
              <strong className="text-yellow-300">
                This shipment is{" "}
                {shipmentStatus?.toLowerCase().replace(/_/g, " ")}.
              </strong>{" "}
            ...
            <section className="bg-gray-900/50 rounded-xl border border-gray-800/50 p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="tel"
                    value={destinationAddress.contactPhone}
                    onChange={setAddr("contactPhone")}
                    disabled={isTerminal}
              ...
              <div className="space-y-2">
                {readOnlyItems.map((item, i) => (
                  ...
                  >
                    ...
                    <span className="text-gray-400 text-sm flex-1 font-mono">
                      {item.skuId}
                    </span>
                    ...
                    {item.notes && (
                      <span className="text-gray-600 text-xs italic truncate max-w-[120px]">
                        {item.notes}
                      </span>
                    )}
                  </div>
